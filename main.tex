...
and 8 threads for all parallel sections (i.e., web workers for \code{kana}, see below).
We collected timings on an Intel Core i7-8850H CPU (2.60GHz, 6 cores, 32 GB memory) running Manjaro Linux.
For convenience, we ran the \code{kana} timings in batch using Puppeteer (\url{https://github.com/puppeteer/puppeteer}) to control a headless Chrome browser (\code{HeadlessChrome/98.0.4758.0}). 
...
Our results indicate that \code{kana} analyses took approximately 25-50\% longer to run compared to the native executable (Table \ref{tab:times}).
...
As an additional evaluation, we recorded the memory usage of the \code{kana} analyses (Table \ref{tab:memory}).
...
Future Wasm releases may support 64-bit pointers (\url{https://github.com/WebAssembly/memory64}) or multiple memories (\url{https://github.com/WebAssembly/multi-memory});
if these proposals are accepted, \code{kana} will be able to process datasets beyond the 4 GB limit.
...
% This indicates that the count matrix is small enough (in its layered sparse form) that it is not the major cause of memory usage in \code{kana}.
...
Inspired by the gallery in \code{Cirrocumulus}, users can save specific views of the embeddings for later perusal.
...
so even if a powerful client device was available, \code{kana} would not be able to fully exploit its capabilities.
...
\begin{itemize}
\item The \code{kana} application is deployed at \url{https://www.jkanche.com/kana} with source code at \url{https://github.com/jkanche/kana}.
\item The \code{scran.js} library is available at \url{https://npmjs.com/package/scran.js} with source code at \url{https://github.com/jkanche/scran.js}.
\item The \code{scran.chan} R package based on the same C++ libraries is available at \url{https://github.com/LTLA/scran.chan}.
