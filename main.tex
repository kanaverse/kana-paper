...
We present kana, a web application for single-cell (scRNA-seq, CITE-seq) data analysis inside the browser.
...
HDF5 files, using either the 10X HDF5 feature barcode matrix format or as H5AD files, SingleCellExperiment \cite{amezquita2020orchestrating} objects stored as RDS files or datasets from public repositories like Bioconductor's ExperimentHub \cite{morgan2019experimenthub} are also supported.
\item We can import more than one dataset into Kana and perform integration or remove batch effects between cells from different samples using MNN correction \cite{haghverdi2018batch}. Options are available to choose an annotation column that specifies batches.
...
\item We can optionally annotate cell types for each computed cluster using references and methods from SingleR \cite{aran2019reference}.
\end{enumerate}
...
\item The SinglePP library \cite {singlepp} adapts singleR methods \cite{aran2019reference} in C++ for cell type annotation.
\item The CppMnnCorrect library \cite{cppmnncorrect} implements the mutual nearest neighbors (MNNs) method  \cite{haghverdi2018batch} for batch correction. It is based on the fastMNN method from the batchelor package \cite{batchelorbioc} in Bioconductor.
\item The rds2cpp library \cite{rds2cpp} to read and write RDS files in C++.
\end{itemize}
...
allowing applications to perform scRNA-seq-related calculations from JavaScript by calling scran.js functions. The bakana library \cite{bakana} used by Kana, wraps scran.js functions into a pipeline to easily run a full single-cell analysis workflow.
