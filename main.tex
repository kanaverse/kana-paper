...
Once the analysis is complete, \code{kana} presents the results in a multi-panel layout (Figure \ref{screenshot:results}).
This contains visualizations for the low-dimensional embeddings, marker tables and a gallery of additional plots.
If a user makes a selection on the embedding, \code{kana} will dynamically compute marker statistics for the selected subset of cells, which can then be viewed on the table.
...
A minor complication with using PThreads is that we need to enable cross-origin isolation on the \code{kana} site.
