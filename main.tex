...
The idea is to use the browser itself to perform statistical and computational analyses directly on the user's machine - in other words, ``client-side compute" - 
and then provide exploratory interface to renders the results of the analysis in an environment highly familiar to the user.

This paradigm of client-side compute has several advantages over the traditional model of server-side data analysis.
...
Each cluster is characterized through differential expression analyses to detect its top markers. We also support many ranking techniques to order the markers genes.
...
Internally, the execution of these steps is encoded as a directed acyclic graph (DAG) to order and organize the dependencies between the steps.
...
They are primarily used for running background tasks in other threads so that the application on the main thread can respond to user interaction.
...
Different panels of the \code{kana} application (Figure \ref{screenshot:results}) can share information with each other to facilitate interactive exploration of the dataset and results of the analysis.
...
This improves application performance by avoiding the rendering bottleneck that would otherwise occur when all plots were drawn at once. 
...
\subsubsection{Format of the saved analysis file}

TODO: use a figure to represent the structure

\begin{itemize} 
\item First 8 bytes specify a 64-bit uinteger in little-endian, specifying the format. Currently its used to denote whether the data files are embedded (0) or linked (1).
\item Next 8 bytes are 64-bit uinteger specifying the format version.
\item Next 8 bytes are 64-bit uinteger specifying the size of the blob containing a gzipped JSON with the analysis parameters and results. Let's call this value n.
\item Next n bytes contain a Gzipped JSON. unzipping this, will contain a property for each step in the analysis and the value is a dictionary with parameters (a dictionary of parameters) and contents (the contents, usually the results).
\item Remaining bytes contain the embedded input files.
\end{itemize}

As users interactively analyze, explore and hypothesize, one may parse the saved analysis in other computational environments (R/Python/notebooks environments) to perform more complex analysis.
...
Most obviously, we are limited by the computational resources available on the client machine,
...
Moreover, we limit \code{kana}'s CPU utilitization to ensure that the analysis does not block user activity.
...
We can easily provide wrappers to the same underlying libraries in any client-side framework, e.g., as a command-line tool (\url{https://github.com/LTLA/scran-cli}) or as a package for existing data analysis ecosystems.
